// markets.js  (página 4: Markets)
// Ruta: "#/markets"
// Server-side pagination con CoinGecko: /coins/markets?vs_currency=usd

const API = 'https://api.coingecko.com/api/v3';

let _abortMarkets = null;
let _page = 1;

export const route = {
  path: /^\/markets$/,   // coincide con "#/markets"
  view: MarketsView
};
...
export async function MarketsView() {
  ...
  const perPage = 50;
  const vsOpts = ['usd', 'eur', 'cop', 'mxn', 'ars', 'brl']; // agrega o quita divisas a gusto
  ...
  const renderSkeleton = () => {
    $app.innerHTML = `
      <h1>Markets</h1>
      <p class="muted">Cargando página ${_page}…</p>
    `;
  };
  ...
  const renderTable = (rows) => {
    $app.innerHTML = `
      ...
      <div class="controls">
        ...
        <select id="vs">
          ${vsOpts.map(opt => `<option value="${opt}" ${opt===vs?'selected':''}>${opt.toUpperCase()}</option>`).join('')}
        </select>
        <button id="refresh">Actualizar</button>
      </div>

      <table class="table" role="table" aria-label="Mercados por capitalización">
        <thead>
          <tr>
            <th>#</th>
            <th>Nombre</th>
    ...
    document.getElementById('vs').addEventListener('change', (e) => {
      vs = e.target.value;
      sessionStorage.setItem('apicoin:vs', vs);
      _page = 1;
      MarketsView();
    });
  };
  ...
  try {
    if (_abortMarkets) _abortMarkets.abort();
    _abortMarkets = new AbortController();
    ...
    $app.innerHTML = `
      <h1>Markets</h1>
      <p class="alert">No se pudo cargar Markets.</p>
      <div class="controls"><button id="retry">Reintentar</button></div>
    `;
