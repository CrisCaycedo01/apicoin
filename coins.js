...
let _coinsCache = null, _abort = null;
...
export async function CoinsView() {
  ...
  try {
    ...
    if (!_coinsCache) {
      ...
      _coinsCache = await res.json();
    }
    let q = '', letter = '', page = 1; const pageSize = 50;
    ...
    const mount = () => {
      ...
      $app.innerHTML = `
        <h1>Coins</h1>
        <div class="controls">
          <input id="q" type="text" placeholder="Buscar..." value="${q}" />
          <select id="letter">
        ...
        <table class="table"><thead><tr><th>#</th><th>Nombre</th><th>Símbolo</th><th>ID</th></tr></thead>
          <tbody>
            ${slice.map((c,i)=>`
              <tr>
                <td>${fmt.num(start+i+1)}</td>
                <td><a href="#/coins/${c.id}">${c.name}</a></td>
    ...
    $app.innerHTML = `<h1>Coins</h1><p class="alert">No se pudo cargar.</p><div class="controls"><button id="retry">Reintentar</button></div>`;
